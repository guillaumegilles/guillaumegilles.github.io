...
[markdown files](markdown.qmd) located in the special `.foam/templates` directory
...
Templates can use all the variables available in [VS Code Snippets](vscode-snippet.qmd).
...
- `filepath`: define a relative or [absolute filepath](absolute-path.qmd)
  to use when creating a note using the template. If the filepath is a
  [relative filepath](relative-path.qmd), it is relative to the current
