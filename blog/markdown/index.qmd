---
title: Markdown Dialect Jungle
draft: true
---

Markdown is an easy-to-read, and easy-to-write plain text format developed
in 2004 by [John Gruber](https://daringfireball.net/projects/markdown/syntax#philosophy)
in collaboration with [Aaron Swartz](https://en.wikipedia.org/wiki/Aaron_Swartz).

The design goal of Markdown's syntax is to make it as readable as possible.
The idea is that a Markdown-formatted document should be publishable as
it is: plain text. The biggest source of inspiration for Markdown’s
syntax is the format of plain text email.

Markdown is not a replacement for [[html]], or even close to it. Its
syntax is very small, only corresponding to a very small subset of HTML
tags. Where HTML is a **publishing** format; Markdown is a **writing
format**.

Markdown is also a free software, written in Perl `Markdown.pl`, that
...
A paragraph is simply one or more consecutive lines of text, separated by one or more blank lines. A blank line is any line that looks like a blank line, i.e. a line containing nothing but spaces or tabs is considered blank. Normal paragraphs should not be indented with spaces or tabs.
...
| To Get…  | Markdown                               |
| -------- | -------------------------------------- |
| - item 1 | `- item 1` or `+ item 1` or `* item 1` |
| - item 2 | `- item 2` or `+ item 2` or `* item 3` |
...
In a regular paragraph, you can create code span by wrapping text in backtick quotes, to get a inline code block:
...
You can produce a horizontal rule tag by placing three or more hyphens `-`, asterisks `*`, or underscores `_` on a line by themselves. If you wish, you may use spaces between the hyphens or asterisks. Each of the following lines will produce a horizontal rule:
...
You can link to a specific section of a different note and have the
content in-lined into the current note.
...
# Markdown's syntax ambiguity

John Gruber’s canonical description of [[markdown]]’s syntax does not [specify the syntax unambiguously](https://spec.commonmark.org/0.31.2/#why-is-a-spec-needed-). In the absence of specifications, early implementers consulted the original `Markdown.pl` (last updated December 17th, 2004) code to resolve these ambiguities. But `Markdown.pl` was quite buggy, and gave manifestly bad
results in many cases, so it was not a satisfactory replacement for a specification.

For this reason, implementations of Markdown have diverged considerably after the initial release of John Gruber. As a result, users are often surprised to find that a document that renders one way on one system (say, a GitHub wiki) renders differently on another (say, converting to docbook using Pandoc). To make matters worse, because nothing in Markdown counts as a “syntax error,” the divergence often isn’t discovered right away.

## Solution

So, in 2014 [John Macfarlane](https://johnmacfarlane.net/), statrted to work on a specification based on his experience writing and maintaining Markdown implementations in several languages, including [[pandoc]], a detailed examination of the differences between existing Markdown implementations using [Babelmark](https://babelmark.github.io/), a tool to compare the output of 20+ implementations of Markdown against each other to see if a consensus emerges. John had extensive discussions with David Greenspan, Jeff Atwood, Vicent Marti,Neil Williams, and Benjamin Dumke-von der Ehe. [[markdown.commonmark]] is the foundation specification for John Gruber's original [[markdown]] work.
...
> [!note]
> Pourquoi break-line at 72 (git commit message) or 80 (PIP8 - IBM punch card) characters?
> 
# History of plain text

1. first file format: .txt 
2. Is there something before that?
3. Then .md and .org
...
- https://pandoc.org/MANUAL.html#pandocs-markdown
- https://github.com/dcurtis/markdown-mark
- https://ia.net/writer/support/basics/markdown-guide
- https://mystmd.org/
...
---
id: zx82d8o99hwj0jvyf7pd532
title: Commonmark
desc: ''
updated: 1729068716815
created: 1729068716815
isDir: false
---
# [CommonMark](https://commonmark.org/)

CommonMark is a [[markdown]] specification trying to solve [[markdown.syntax-ambiguity]].
It propose a **standard, unambiguous syntax specification for Markdown**, along
with a suite of comprehensive tests to validate Markdown implementations against
this specification.

This project originated in 2014, with:

- **John MacFarlane**, of [Pandoc](https://pandoc.org/)
- **David Greenspan**, of [Meteor](https://www.meteor.com/)
- **Vicent Marti**, of [GitHub](https://github.com/)
- **Neil Williams**, of [Reddit](https://www.reddit.com/)
- **Benjamin Dumke-von der Ehe**, of [Stack Overflow](https://stackoverflow.com/)
- **Jeff Atwood**, of [Discourse](https://www.discourse.org/)

CommonMark serves as a baseline for other Markdown flavor:

- [[markdown.github-flavored]]
- [[markdown.pandoc]]

---
id: w9r43lkl0293yavd66fdcw2
title: GitHub Flavored Markdown
desc: 'GFM'
updated: 1730124949841
created: 1729068716826
---

## [[github]] Flavored [[markdown]]

https://github.github.com/gfm/, often shortened as GFM, is the dialect of [[markdown]] that is currently supported for user content on [GitHub](https://github.com/)

It is based on the [[markdown.commonmark]] and build upon it to add new features to the original Markdown, such as:

- [Tables](#tables)
- [Task list](#task-list-items)
...
2. A delimiter row separating the header.
3. And zero or more data rows.
...
[[html.block-level-elements]], like paragraphs or header cannot be inserted in a table.
...
Autolinks create a [[hyperlink]] automatically.
...
Filtering is done by replacing the leading `<` with the entity `&lt;`. These tags are chosen in particular as they change how HTML is interpreted in a way unique to them (i.e. nested HTML is interpreted differently), and this is usually undesireable in the context of other rendered Markdown content. All other HTML tags are left untouched.

---
id: fj9r8p3gmofr7i6ju72tfqg
title: Callout Blocks in GFM
desc: ''
updated: 1730125072921
created: 1730124989615
---

To create a callout in [markdown.github-flavored], add [!info] to the first line of a blockquote, where info is the _type identifier_.
...
The type identifier determines how the callout looks and feels. Available types are:
...
`LaTeX`est un système de format de document développé par faciliter
l'écriture de formules mathématiques dans les publications techniques
scientifiques
...
| $x \cdot y$   | multiplication with dot  | `$x \cdot y$`  |
| $x^{2}$       | exposant                 | `$x^{2}$`       |
| $x_{i}$       | indidce                  | `$x_{i}$`       |
...
---
id: ztsr1pw1bsalypfxkme54sz
title: Chronology
desc: ''
updated: 1729068716832
created: 1729068716832
isDir: false
title_imported: Chronology of Literate Programming
---

| Année | Tech                     | Author             |
| ----- | ------------------------ | ------------------ |
| 1978  | TeX                      | Donald Knuth       |
| 1984  | [[cs.prog.literate]]     | Donald Knuth       |
| 1988  | Mathematica Notebooks    | Stephem Wolfram    |
| 2001  | [[python.ipython]]                  | Fernando Perez     |
| 2002  | Sweave                   | Friedrich Leisch   |
...
| 2009  | [[markdown.github-flavored]] | Tom Preston-Werner |
| 2011  | iPython Notebook         | Fernando Perez     |
| 2012  | knitr + [[rmarkdown]]    | Yihui Xie          |
| 2014  | [[jupyter]]          | Fernando Perez     |
| 2022  | [[quarto]]               | Joseph J. Allaire  |


---
id: d63fkshq4c4y6z4771f1ocn
title: Literate
desc: ''
updated: 1729068716833
created: 1729068716833
tags: []
isDir: false
title_imported: Literate Programming
type_imported: null
---

Donald E. Knuth, author of "The art of computer programming", wrote a paper in
1984, _literate programming_, which lay the ground for a new [[programming-paradigm]].

He insists that programs are "_works of literature_". Instead of focusin on how
to "to instruct a computer what to do, let us concentrate rather on explaining
to human beings what we want a computer to to. [Therefore] the practitioner of
literate programming can be regarded as an essayist, whose main concern is with
**exposition** and **excellence of style**." the link beteween **literate programming**
and [[cs.prog.good-code]] is quite easy to make.

## The `WEB` System

"`WEB` itself is a combination of two other languages: (1) a document formatting
language and (2) a programming language. [...] The main point is that `WEB` is
inherently bilingual. It doesn't make the other languages obsolete: on the contrary,
it enhances them."

The `WEB` system uses TeX for document formatting and `PASCAL` as a programming language.
A `WEB` user writes a program that serves as the source language for two different
system routines. One line of processing is called _weaving_the web; it produces
a document that describes the program clearly and that facilitates program
maintenance. The other line of processing is called _tangling_ the web: it produces
a machine executable program. The program and its documentation are both generated
from the source, so they are consistent with each other.

```mermaid
flowchart LR;
  web([WEB]) -->|WEAVE|tex([TEx]);
  web([WEB]) -->|TANGLE|pas([PAS]);
  tex([TEx]) -->|TeX|dvi([DVI]);
  pas([PAS]) -->|PASCAL|rel([REL]) ;
```

[[literate-programming-chronology|since-then]], Literate programming has
influenced many data science software, like [[jupyter]] or [Google Colab](https://colab.google/).
Nowadays, [[quarto]] replaces the `WEB` system with [[markdown]] as a formatting
language and [[r]] or [[python]] as a programming language.
...
Desire for [[cs.prog.literate]] and reproductible research in the scientific
community. One option is to leverage the power of [[org-mode]].
...
---
id: suvttorj4f0st0hbw6ogdey
title: Pandoc Markdown
desc: ''
updated: 1730716230938
created: 1729068716841

---
# [Pandoc’s Markdown](https://pandoc.org/MANUAL.html#pandocs-markdown)

Pandoc's Markdown is a dialect of [[markdown]] created by John MacFarlane (see [[markdown.commonmark]]) adding new features to the original Markdown:

- 

In creating Pandoc's Markdown, John MacFarlane diffes slyttly from
[[markdown]] Phylosophy. Whereas Markdown was originally designed with
HTML generation in mind, pandoc is designed for multiple output formats.
Thus, while pandoc allows the embedding of raw HTML, **Pandoc's Markdown
...
An ATX-style heading consists of one to six # signs and a line of text, optionally followed by any number of `#` signs.
The number of `#` signs at the beginning of the line is the heading level:
...
- Publish reproducible, production quality articles, [[quarto.revealjs]],
  dashboards, [[quarto.website]], blogs, and books in HTML,
  PDF, MS Word, ePub, and more.
- Share knowledge and insights organization-wide by publishing to Posit
  Connect, Confluence, or other publishing systems.
...
processed by [[jupyter]], and the resulting combination of code, markdown,
and output is converted to plain markdown. Then, this markdown is processed
by [[pandoc]], which creates the finished format.

![Quarto How It Works](/assets/qmd-workflow.png)
...
To include citations inside a page markdwon syntax : `@citation`

1. By default, quarto include a list of citations references at the end
of a rendered document

2. You can specify a precise location of the document using
...
3. You can interly modify the location with the YAML options: 
`citation-location: margin`. Actually the default option for this is
`document` which create the rule number 1