...
```mermaid
graph TD
    subgraph "Agentic Engineering Ecosystem"
        OR["OpenResponses &<br/>Interactions API<br/>(Power Plugs)"]
        MCP["MCP<br/>(USB-C)"]
        SK["Skills<br/>(Playbooks)"]
        A2A["A2A<br/>(Factory Radio)"]
        A2UI["A2UI<br/>(Generative Display Window)"]
        AP2UCP["AP2 & UCP<br/>(Global Supply Chain<br/>& Transaction Network)"]
    end

    OR -->|Long-running tasks| LLM["LLM Inference"]
    MCP -->|Tools & Data| LLM
    SK -->|Sandbox execution| LLM
    A2A <-->|Agent negotiation| A2A
    A2UI -->|Interactive UI| Human
    AP2UCP -->|Commerce| Merchant

    style OR fill:#e1f5fe
    style MCP fill:#e1f5fe
    style SK fill:#e1f5fe
    style A2A fill:#e1f5fe
    style A2UI fill:#e1f5fe
    style AP2UCP fill:#e1f5fe
```
...
```mermaid
flowchart LR
    A["1. Discovery<br/>Find pre-built MCP servers<br/>from public, 3P, or<br/>internal registries"] --> B["2. Configuration<br/>Set environment vars,<br/>scope, permissions,<br/>auth credentials"]
    B --> C["3. Connection<br/>Client handshake,<br/>list tools, validate<br/>output schema"]
    C --> D["✅ Ready<br/>Agent can use tools"]
```
...
```mermaid
graph LR
    subgraph Traditional[NxM Integration]
        A1[Model A] --> T1[Tool 1]
        A2[Model B] --> T2[Tool 2]
        A3[Model C] --> T3[Tool 3]
        A1 -.-> T2
        A2 -.-> T1
    end

    subgraph MCP[MCP Interoperability]
        M1[Model A] --> Hub[MCP]
        M2[Model B] --> Hub
        M3[Model C] --> Hub
        Hub --> X1[Tool 1]
...
```mermaid
graph TD
    subgraph "Single Runtime / Shared Memory"
        ORCH[Orchestrator Agent]
        DB_AGENT[DB Sub-Agent<br/>Tools: query, schema]
        UI_AGENT[UI Sub-Agent<br/>Tools: render, layout]
        BILL_AGENT[Billing Sub-Agent<br/>Tools: invoice, payment]
        TEST_AGENT[Testing Sub-Agent<br/>Tools: assert, coverage]
    end
    ...
    DB_AGENT --> DB[(Database)]
    UI_AGENT --> UI[UI Layer]
    BILL_AGENT --> PAY[Payment Gateway]
    TEST_AGENT --> LOGS[Test Logs]

    style ORCH fill:#fff3e0
    style DB_AGENT fill:#e3f2fd
    style UI_AGENT fill:#e3f2fd
    style BILL_AGENT fill:#e3f2fd
...
**Figure 4: Distributed Multi-Agent Architecture**
...
graph TD
    ...
    subgraph "Network Boundary"
        GOOGLE_AGENT[Google Agent<br/>Python / ADK]
        SF_AGENT[Salesforce Agent<br/>LangChain]
        WD_AGENT[Workday Agent<br/>Bespoke]
    end

    USER[User] -->|"Intent"| ORCH
    ORCH -->|"A2A Protocol"| GOOGLE_AGENT
    ORCH -->|"A2A Protocol"| SF_AGENT
    ORCH -->|"A2A Protocol"| WD_AGENT
    
    GOOGLE_AGENT --> GCP[Google Cloud Services]
    SF_AGENT --> CRM[Salesforce CRM]
    WD_AGENT --> HR[Workday HR]

    style ORCH fill:#fff3e0
    style GOOGLE_AGENT fill:#e8f5e9
    style SF_AGENT fill:#e8f5e9
    style WD_AGENT fill:#e8f5e9
```
...
**Figure 5: A2A Server/Client**

```mermaid
flowchart LR
    subgraph Supply[Supply Side]
        NATIVE[Native AI Agent<br/>ADK / LangGraph / Bespoke]
        EXEC[Agent Executor<br/>Translation Layer]
        CARD[Agent Card<br/>Capabilities, Security, Schemas]
        ENDP[A2A Endpoint]
    end
...
**Figure 6: Agent-as-a-Service Lifecycle**

```mermaid
flowchart LR
    DEV[Independent Vendor<br/>/ Developer] -->|"Lists A2A Agent"| MP[Google Cloud<br/>Marketplace]
    MP --> ENTERPRISE[Enterprise Customers]
    ENTERPRISE -->|"Procurement +<br/>GCP Commitments"| MP
    MP -->|"Hybrid Pricing<br/>Flat Fee + Usage"| VENDOR_REV[Revenue<br/>for Vendor]
    MP -->|"Infrastructure<br/>Billing"| GCP[GCP Platform]

    style DEV fill:#e8f5e9
    style MP fill:#fff3e0
    style ENTERPRISE fill:#e3f2fd
    style VENDOR_REV fill:#e8f5e9
...
| Interactive as  | Button, TextField, CheckBox, Slider, DateTimeInput, ChoicePicker |
...
# Wire the converter at executor setup so this tool's response becomes an A2UI part:
...
sequenceDiagram
    ...
    User->>Agent: "Show me Q4 sales by region"
    Agent->>Canvas: Render A2UI dashboard
    Canvas-->>User: Interactive dashboard displayed
    User->>Canvas: Edits a value / clicks filter
    Canvas->>Agent: Reports interaction event
    Agent->>Canvas: Updates dashboard in real-time
...
# 2. Run the agent. It emits text containing one or more <a2ui-json>...</a2ui-json>
#    blocks; parse_response() extracts the parsed JSON. Validate, then retry on errors.
...
Lab recommendations: https://codelabs.developers.google.com/next26/adk-agent-commerce#0
...
graph LR
    subgraph Consumer[Consumer Side]
        USER[User]
        AI[AI Smart Assistant]
        MANDATE[Digital Mandate<br/>Spending Rules]
    end

    subgraph Commerce[Commerce Layer]
        UCP[Universal Commerce Protocol<br/>Menu, Inventory, Ordering]
        AP2[Agent Payments Protocol<br/>Payment, Authorization]
    end

    subgraph Merchant[Merchant Side]
        STORE[Store / Restaurant]
        BANK[Payment Processor / Bank]
    end

    USER -->|"Approve rules"| MANDATE
    MANDATE --> AI
    AI --> UCP
    UCP --> STORE
    AI --> AP2
...
| Unified integration:                                       | Authorization & Auditability                                     |
| Shared language:                                           | Authenticity of Intent                                           |
| Extensible architecture                                    | Agent Error and Hallucination                                    |
| Security-first approach                                    | Accountability                                                   |
